...
/* square macro */
#define POW2(a) ((a)*(a))
/* number of cost function parameters */
#define NPCOST 2
...
{
    typeInt idx = t / param->dt;
    typeRNum* pCost = (typeRNum*)userparam;
	typeRNum* y = pCost + NPCOST;
    
    if(param->t0 + t - param->Thor >= 0) {
	    out[0] = 0.5 * pCost[1] * POW2(p[0] + x[0] - y[idx]);
    }
}
/** Gradient dl/dx **/
...
void Vfct(typeRNum *out, ctypeRNum T, ctypeRNum *x, ctypeRNum *p, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
    ...
    if(param->t0 >= 0) {
	    out[0] = 0.5 * pCost[0] * POW2(p[0] + x[0] - y[idx]);
    }
}
