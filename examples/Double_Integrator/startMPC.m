...
probfct = 'probfct_DOUBLE_INTEGRATOR.c';
...
while 1
    ...
    % stop time optimization if minimal end time is reached
    if (grampc.sol.Tnext <= grampc.param.Tmin + grampc.param.dt) && (grampc.opt.OptimTime == 1)
        grampc = CmexFiles.grampc_setopt_Cmex(grampc, 'OptimTime', 'off');
    ...
    % plot data
    if figNr > 0 && (mod(i,PLOT_STEPS) == 0 || vec.t(i+1) >= Tsim)
        if PLOT_PRED
...
%% Adjust x axis
if figNr > 0
    figure(figNr-1),xlim([vec.t(1),vec.t(i)])
    figure(figNr-2),xlim([vec.t(1),vec.t(i)])
end
end
