...
{
    ctypeRNum dt = param->dt;
    ctypeRNum u_diff = (u[0] - u[1]);
    ctypeRNum u_add = (u[0] + u[1]);
    ctypeRNum dt_2 = POW2(dt);

	out[0] = x[0] + dt * x[2] + 0.095 * dt_2 * u_add;
	out[1] = x[1] + dt * x[3] + (0.01 * dt + 0.66 * dt_2) * u_diff;
	out[2] = x[2] + 0.19 * dt * u_add;
	out[3] = x[3] + 1.32 * dt * u_diff;
	out[4] = x[4] + dt * x[0] + dt_2 * 0.5 * x[3] + 0.0317 * POW(dt, 3.0) * u_add;
	out[5] = x[5] + dt * x[1] + dt_2 * 0.5 * x[4] + (0.22 * dt + 0.005) * dt_2 * u_diff;
}
...
void dfdx_vec(typeRNum *out, ctypeRNum t, ctypeRNum *x, ctypeRNum *u, ctypeRNum *p, ctypeRNum *vec, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[0] = vec[0] + dt * vec[4];
	out[1] = vec[1] + dt * vec[5];
	out[2] = vec[2] + dt * vec[0];
	out[3] = vec[3] + dt * vec[1] + dt_2 * 0.5 * vec[4];
	out[4] = vec[4] + + dt_2 * 0.5 * vec[5];
	out[5] = vec[5];
}
...
void dfdu_vec(typeRNum *out, ctypeRNum t, ctypeRNum *x, ctypeRNum *u, ctypeRNum *p, ctypeRNum *vec, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[0] = 0.095 * dt_2 * vec[0] 
        + (0.01 * dt + 0.66 * dt_2) * vec[1] 
        + 0.19 * dt * vec[2] 
        + 1.32 * dt * vec[3] 
        + 0.0317 * dt_2 * dt * vec[4] 
        + (0.22 * dt + 0.005) * dt_2 * vec[5];
	out[1] = 0.095 * dt_2 * vec[0] 
        - (0.01 * dt + 0.66 * dt_2) * vec[1] 
        + 0.19 * dt * vec[2] 
        - 1.32 * dt * vec[3] 
        + 0.0317 * dt_2 * dt * vec[4] 
...
/** Terminal cost V(T,x(T),p,userparam) 
    ---------------------------------------- **/
