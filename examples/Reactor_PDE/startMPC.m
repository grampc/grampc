...
% This function runs the reactor PDE example. It compiles the c-Code,
...
odeopt =  odeset('Mass',Mfct,'RelTol',1e-6,'AbsTol',1e-8);
...
%% Additional code
if figNr > 0 && PLOT_MESH == 1
    figure(figNr)
    clf;
    figNr = figNr+1;
    mesh(0:10,vec.t,vec.x')
    view([50,34]),xlabel('Length z'),ylabel('Time t'),zlabel('State')
end
end
