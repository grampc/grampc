 ...
 *
 *
 * This probfct-file describes the VTOL problem from
 * Sastry, S.: Nonlinear Systems: Analysis, Stability and Control.
 * Springer Science & Business Media, New York, USA (2013)
 *
 *                                           _T
 *                                          /
...
void dfdx_vec(typeRNum *out, ctypeRNum t, ctypeRNum *x, ctypeRNum *u, ctypeRNum *p, ctypeRNum *vec, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[2] = 0;
	out[3] = vec[2];
	out[4] = (-(COS(x[4])*u[0]) - pSys[0] * SIN(x[4])*u[1])*vec[1] + (-(SIN(x[4])*u[0]) + COS(x[4])*pSys[0] * u[1])*vec[3];
	out[5] = vec[4];
}
...
void dfdu_vec(typeRNum *out, ctypeRNum t, ctypeRNum *x, ctypeRNum *u, ctypeRNum *p, ctypeRNum *vec, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[0] = -(SIN(x[4])*vec[1]) + COS(x[4])*vec[3];
	out[1] = COS(x[4])*pSys[0] * vec[1] + pSys[0] * SIN(x[4])*vec[3] + vec[5];
}
...
{
	ctypeRNum * pCost = (ctypeRNum*)userparam + 1;
    ctypeRNum *xdes = param->xdes;
    ctypeRNum *udes = param->udes;

	out[0] = pCost[12] * POW2(u[0] - udes[0])
		+ pCost[13] * POW2(u[1] - udes[1])
		+ pCost[6] * POW2(x[0] - xdes[0])
		+ pCost[7] * POW2(x[1] - xdes[1])
		+ pCost[8] * POW2(x[2] - xdes[2])
		+ pCost[9] * POW2(x[3] - xdes[3])
		+ pCost[10] * POW2(x[4] - xdes[4])
		+ pCost[11] * POW2(x[5] - xdes[5]);
}
/** Gradient dl/dx **/
