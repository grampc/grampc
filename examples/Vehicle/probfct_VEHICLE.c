...
void ffct(typeRNum *out, ctypeRNum t, ctypeRNum *x, ctypeRNum *u, ctypeRNum *p, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[0] = COS(x[2])*x[4];
	out[1] = SIN(x[2])*x[4];
	out[2] = (x[3] * x[4]) / (pSys[0] * (1 + 1 / POW2(pSys[1])*POW2(x[4])));
	out[3] = u[0];
	out[4] = u[1];
}
...
void dfdx_vec(typeRNum *out, ctypeRNum t, ctypeRNum *x, ctypeRNum *u, ctypeRNum *p, ctypeRNum *vec, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[3] = (vec[2] * POW2(pSys[1])*x[4]) / (pSys[0] * POW2(pSys[1]) + pSys[0] * POW2(x[4]));
	out[4] = vec[0] * COS(x[2]) + vec[1] * SIN(x[2]) + (vec[2] * POW2(pSys[1])
		/ POW2(POW2(pSys[1]) + POW2(x[4]))*x[3] * (pSys[1] - x[4])*(pSys[1] + x[4])) / pSys[0];
}
...
void lfct(typeRNum *out, ctypeRNum t, ctypeRNum *x, ctypeRNum *u, ctypeRNum *p, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[0] = pCost[12] * POW2(u[0] - udes[0])
		+ pCost[13] * POW2(u[1] - udes[1])
		+ pCost[2] * POW2(x[0] - xdes[0])
		+ pCost[3] * POW2(x[1] - xdes[1])
		+ pCost[4] * POW2(x[2] - xdes[2])
		+ pCost[5] * POW2(x[3] - xdes[3])
		+ pCost[6] * POW2(x[4] - xdes[4]);
}
/** Gradient dl/dx **/
void dldx(typeRNum *out, ctypeRNum t, ctypeRNum *x, ctypeRNum *u, ctypeRNum *p, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[0] = 2 * pCost[2] * (x[0] - xdes[0]);
	out[1] = 2 * pCost[3] * (x[1] - xdes[1]);
	out[2] = 2 * pCost[4] * (x[2] - xdes[2]);
	out[3] = 2 * pCost[5] * (x[3] - xdes[3]);
	out[4] = 2 * pCost[6] * (x[4] - xdes[4]);
}
/** Gradient dl/du **/
...
void Vfct(typeRNum *out, ctypeRNum T, ctypeRNum *x, ctypeRNum *p, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[0] = pCost[7] * POW2(x[0] - xdes[0])
		+ pCost[8] * POW2(x[1] - xdes[1])
		+ pCost[9] * POW2(x[2] - xdes[2])
		+ pCost[10] * POW2(x[3] - xdes[3])
		+ pCost[11] * POW2(x[4] - xdes[4]);
}
/** Gradient dV/dx **/
void dVdx(typeRNum *out, ctypeRNum T, ctypeRNum *x, ctypeRNum *p, const typeGRAMPCparam *param, typeUSERPARAM *userparam)
{
	...
	out[0] = 2 * pCost[7] * (x[0] - xdes[0]);
	out[1] = 2 * pCost[8] * (x[1] - xdes[1]);
	out[2] = 2 * pCost[9] * (x[2] - xdes[2]);
	out[3] = 2 * pCost[10] * (x[3] - xdes[3]);
	out[4] = 2 * pCost[11] * (x[4] - xdes[4]);
}
/** Gradient dV/dp **/
